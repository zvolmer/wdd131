const recipes = [
  ...
  },
  {
    ...
    recipeInstructions: [
      ...
      'Pour in cream, mustard and salt and pepper.  Stir.  On medium heat, simulate until it thickens (5 minutes)',
...
function tagsTemplate(tags) {
  return `
    <ul class="recipe__tags">
      ${tags.map((tag) => `<li class="recipe__tag">${tag}</li>`).join('')}
    </ul>
  `;
}
...
function filterRecipes(query) {
  ...
  const filtered = recipes.filter((r) => {
    ...
    const inDesc = r.description.toLowerCase().includes(lower);
    const inTags = r.tags.some((t) => t.toLowerCase().includes(lower));
    const inIngredients = r.recipeIngredient.some((ing) =>
